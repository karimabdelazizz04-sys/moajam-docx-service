...
@app.route("/")
def home():
    return "Moajam DOCX Render Service Running"
...
def generate_docx():
    ...
    if isinstance(layout_plan, str):
        try:
            layout_plan = json.loads(layout_plan)
        except Exception:
            layout_plan = {}

    if not letterhead_url:
        return jsonify({"status": "error", "message": "Missing letterhead_image_link"}), 400
    if not translated_text and not (isinstance(layout_plan, dict) and layout_plan.get("blocks")):
        return jsonify({"status": "error", "message": "Missing translated_text/layout_plan_json"}), 400

    try:
        letterhead_path = download_file(letterhead_url)
        doc = build_docx(translated_text, layout_plan, letterhead_path)
...
def build_docx(translated_text, layout_plan, letterhead_path):
    doc = Document()
